...
public class Main493 {
    ...
    public void solve() {
    	int n = ini();
    	long a[] = in(n);
    	OfflineTree tree = new OfflineTree(a);
    	OfflineTreeMax max = new OfflineTreeMax(a);
    	TreeMap<Long, Long> map = new TreeMap<>();
    	fillmap(map, a);
    	for(Map.Entry<Long, Long> entry : map.entrySet()) tree.update(entry.getKey(), entry.getValue());
    	long dp[] = new long[map.size()];
    	int index = 0;
    	for(Map.Entry<Long, Long> entry : map.entrySet()) {
    		dp[index] += entry.getValue() + max.query(-10, entry.getKey() - 2);
    		max.update(entry.getKey(), dp[index]);
    		index++;
    	}
    	pl(dp[max(dp)]);
    }   
